...
func (q *Queries) GetAllChirps(ctx context.Context, isdescorder bool) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirps, isdescorder)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetChirpsByAuthor(ctx context.Context, arg GetChirpsByAuthorParams) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChirpsByAuthor, arg.UserID, arg.Isdescorder)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
